...
def _optimized_hlo(fn, *args) -> str:
    ...
    PJRT_Executable_OptimizedProgram in the plugin.
    """
...
def test_optimized_program_is_available():
    """The API is implemented at all (used to return UNIMPLEMENTED)."""
    text = _optimized_hlo(lambda x: x + 1.0, jnp.ones((4,), jnp.float32))
    assert "HloModule" in text
    assert "ENTRY" in text


def test_optimized_program_reflects_simplification():
    ...
    text = _optimized_hlo(lambda v: v * 1.0 + 2.0, x)
...
def test_optimized_program_reflects_mps_fusion():
...
def test_optimized_program_is_stable_across_calls():
...
def test_optimized_program_survives_execution():
...
def test_optimized_program_across_shapes(fn, args):
